...
def backup_func():
    ...
    with open(file, "rb") as f:
        ...
        na_bot.sendMessage(backups_chat, f"Backup automatico concluido: {cstrftime}\nDownload: {url}")
    os.remove(file)
        ...
        for resutado in resultados:
            ...
            try:
                if hoje[0:2] == aviso[0:2]:
                    na_bot.sendMessage(id_grupo,
                                       f"🤖 {doador} ***Falta uma semana para você grupo, caso ainda tenha interesse em continuar usando a loja faça uma doação, envie o comprovante aqui no grupo que um de nossos administradores irá colocar mas dias em sua permanencia.***\n`Usuário:` {doador}\n`Id_Usuário:` {id_doador}\n`Início:` {data_inicial}\n`Termino:` {data_ban}\n`Permanência:` {dias}",
                                       'markdown')
                ...
                if hoje[3:5] == data_ban[3:5]:
                    ...
                    na_bot.sendMessage(str(id_grupo),
                                       f"🤖 ***Removido do grupo pois deu a sua permanência do grupo.***\n`Usuário:` {doador}\n`Id_Usuário:` {id_doador}\n`Início:` {data_inicial}\n`Termino:` {data_ban}\n`Permanência:` {dias}",
                                       'markdown')
